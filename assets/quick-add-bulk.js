      ...
      connectedCallback() {
        this.cartUpdateUnsubscriber = subscribe(PUB_SUB_EVENTS.cartUpdate, (event) => {
          if (
            ...
            // FIX: event.cartData can be undefined when the cart update originates
            // from standard-actions-override.js and the /cart.js fetch failed or
            // returned non-ok. Without optional chaining, accessing .items on
            // undefined throws TypeError, which propagated through pubsub's
            // Promise.all() and triggered window.location.reload() in the
            // Standard Actions updateCart handler. Use optional chaining (?.)
            // so a missing cartData is treated as "no items match this index"
            // and the component refreshes itself via onCartUpdate() as a safe
      ...
      renderSections(parsedState, ids) {
        ...
        this.getSectionsToRender().forEach((section) => {
          ...
          if (section.section === 'cart-drawer') {
            // FIX: parsedState.items.length.length was always undefined (number
            // has no .length property), making this condition always false.
            // The correct check is parsedState.items.length === 0.
